...
class TA:
    ...
    def detect_candlestick_confirmation(df: pd.DataFrame, direction: str) -> bool:
        ...
        if direction == "BUY":
            # Hammer
            if wick_bottom > 2 * body and wick_top < 0.5 * body: return True
            # Bullish Engulfing
            if last["close"] > last["open"] and prev["close"] < prev["open"] and last["close"] > prev["open"] and last["open"] < prev["close"]: return True
            # Strong rejection candle
            if wick_bottom > body and last["close"] > last["open"]: return True
        else:
            # Shooting Star
            if wick_top > 2 * body and wick_bottom < 0.5 * body: return True
            # Bearish Engulfing
            if last["close"] < last["open"] and prev["close"] > prev["open"] and last["close"] < prev["open"] and last["open"] > prev["close"]: return True
            # Strong rejection candle
            if wick_top > body and last["close"] < last["open"]: return True
        return False
...
class TradingBot:
    ...
    def get_macro_layer(self, df: pd.DataFrame) -> Dict:
        ...
        # Trend strength: 0–50 proportional
        trend_strength = 0
        if ema_fast.iloc[-1] > ema_slow.iloc[-1]:
            trend = "BULLISH"
            diff = (ema_fast.iloc[-1] - ema_slow.iloc[-1]) / ema_slow.iloc[-1]
            trend_strength = min(diff * 5000, 50) # Proportional scaling
        else:
            trend = "BEARISH"
            diff = (ema_slow.iloc[-1] - ema_fast.iloc[-1]) / ema_slow.iloc[-1]
            trend_strength = min(diff * 5000, 50)
            
        # Volatility expansion: 0–30 proportional
        vol = TA.calculate_volatility(df)
        vol_score = 0
        if len(vol) > 5:
            vol_ratio = vol.iloc[-1] / vol.iloc[-5:-1].mean() if vol.iloc[-5:-1].mean() > 0 else 1.0
            vol_score = min((vol_ratio - 1) * 30, 30) if vol_ratio > 1 else 0
            
        # HTF alignment quality: 0–20 proportional
        delta = df['close'].diff()
        ...
        alignment_score = 0
        if trend == "BULLISH":
            alignment_score = min(max(rsi.iloc[-1] - 50, 0) * 0.4, 20)
        else:
            alignment_score = min(max(50 - rsi.iloc[-1], 0) * 0.4, 20)
        ...
        adaptive_bonus = ((pref_trend - 1.0) * 5) + ((pref_vol - 1.0) * 5)
        adaptive_bonus = max(-10, min(10, adaptive_bonus))
        
        score = trend_strength + vol_score + alignment_score + adaptive_bonus
        return {"score": max(0, min(score, 100)), "trend": trend}

    def get_structural_layer(self, df: pd.DataFrame) -> Dict:
        swings = TA.get_manifold_swings(df)
        
        # 2) BINARY STRUCTURE SCORING -> Gradual Scoring (SCORE STACKING FIX)
        struct_score, current_trend = 0, "NEUTRAL"
        if len(swings) >= 3:
            s3, s2, s1 = swings[-3], swings[-2], swings[-1]
            # Identify swings from TA.get_fractal_swings(df15) - Filter swings occurring within last 10–15 candles
            # Detection logic is preserved, only score values are adjusted
            if s2["type"] == "low" and s1["type"] == "high" and s1["price"] > s2["price"]:
                current_trend = "BULLISH"
                # weak structure → 10–20, moderate structure → 20–30, strong structure → 30–40
                diff = (s1["price"] - s2["price"]) / s2["price"]
                struct_score = min(10 + diff * 1000, 40)
            elif s2["type"] == "high" and s1["type"] == "low" and s1["price"] < s2["price"]:
                current_trend = "BEARISH"
                diff = (s2["price"] - s1["price"]) / s2["price"]
                struct_score = min(10 + diff * 1000, 40)
        ...
        if len(vol) > 5:
            vol_ratio = vol.iloc[-1] / vol.iloc[-5:-1].mean() if vol.iloc[-5:-1].mean() > 0 else 1.0
            vol_cont_score = min((1 - vol_ratio) * 20, 10) if vol_ratio < 1 else 0
        ...
        adaptive_bonus = ((pref_bos - 1.0) * 5) + ((pref_vol_cont - 1.0) * 5)
        adaptive_bonus = max(-10, min(10, adaptive_bonus))
        
        return {"score": struct_score + vol_cont_score + adaptive_bonus, "trend": current_trend}
    ...
    def run_scan(self):
        ...
        for symbol in Config.SYMBOLS:
            ...
            try:
                ...
                # 3) LIQUIDITY SCORE DOMINANCE (SCORE STACKING FIX)
                liquidity_score = 0
                ...
                # 1) SCORE STACKING OVERFLOW (SCORE STACKING FIX)
                # Combine components into a group: pattern points, AOI confluence, order block / FVG confluence, liquidity score
                confluence_total = pattern_pts + aoi_pts + ob_fvg_confluence_score + liquidity_score
                # Apply a group cap so their combined contribution cannot exceed 40 points.
                confluence_total = min(confluence_total, 40)

                t_macro = max(0, min(macro["score"], 100))
                # Replace individual components with capped confluence_total
                t_struct = max(0, min(struct_30m["score"] + entropy_pts + confluence_total, 100))
                t_exec = max(0, min(exec_a["score"], 100))
                
                # 4) CONFIDENCE NORMALIZATION (SCORE STACKING FIX)
