...
class TradingBot:
        ...
        for symbol in Config.SYMBOLS:
            ...
            try:
                ...
                if not htf_aligned:
                    ...
                    # Capture rejection even if HTF fails
                ...
                # PART 2 — MANDATORY CANDLESTICK CONFIRMATION AT AOI
                ...
                # PART 3 — SEMI-REQUIRED LIQUIDITY SWEEP
                ...
                if liquidity_sweep_confirmed:
                    # Positive score boost if sweep detected
                ...
                # FIX 3 — CLEAN BOS / RETEST LOGIC
                bos_occurred = TA.detect_micro_bos(df_15m, exec_a["direction"])
                pullback_continuation_confirmed = False
                if bos_occurred:
                    # IF a BOS occurs, THEN a retest of the broken structure level becomes MANDATORY before entry.
                    ...
                    # If no BOS exists, normal structure rules apply.
    ...
    def _calculate_trade_params(self, df15, df30, df2h, direction) -> Tuple[float, float, float]:
        ...
        # FIX 1 — STOP LOSS MUST USE NEAREST RECENT SWING (NOT GLOBAL)
        # Filter swings occurring within last 15 candles
        recent_swings = [s for s in sw15 if s["index"] >= len(df15) - 15]
        
        if direction == "BUY":
            # From those, select the nearest valid swing relative to entry
            valid_lows = [s["price"] for s in recent_swings if s["type"] == "low" and s["price"] < entry]
            if valid_lows:
                # Nearest means the highest low that is still below entry
                sl = max(valid_lows) - 0.2 * atr15
            else:
                return entry, 0, 0 # Reject if no recent swing low
        else:
            valid_highs = [s["price"] for s in recent_swings if s["type"] == "high" and s["price"] > entry]
            if valid_highs:
                # Nearest means the lowest high that is still above entry
                sl = min(valid_highs) + 0.2 * atr15
            else:
                return entry, 0, 0 # Reject if no recent swing high
            
        # FIX 2 — REMOVE RR-BASED TP FALLBACK COMPLETELY
        ...
        if direction == "BUY":
            ...
            if not targets: return entry, 0, 0 # Reject if no valid 30M target
            tp = min(targets)
            # BUT must not exceed 2H resistance
            ...
            if not targets: return entry, 0, 0 # Reject if no valid 30M target
            tp = max(targets)
            # BUT must not exceed 2H support
