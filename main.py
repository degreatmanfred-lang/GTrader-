...
class ToolboxPreference:
    def __init__(self):
        ...
        # component_state âˆˆ {ACTIVE, SUSPENDED}
    ...
    def adjust_preference(self, component_name: str, adjustment: float, smoothing_factor: float = 0.02):
        ...
        print(f"Adjusted preference for {component_name}: {current_pref:.4f} -> {self.preferences[component_name]:.4f} (Î”: {delta:.4f})")
    ...
    def record_trade_outcome(cls, outcome_data: Dict):
        ...
        if db:
            try:
                ...
                db.collection("trade_outcomes").add(outcome_data)
                print(f"Trade outcome recorded: {outcome_data['symbol']}")
            except Exception as e:
        ...
        for p in prices:
            ...
            if touches >= 3 and rejections >= 2:
                ...
                aois.append({
                    ...
                    "touches": touches, "rejections": rejections, "rejection_strength": rejection_strength
                })
        return aois
