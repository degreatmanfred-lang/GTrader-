...
class TradingBot:
        ...
        for symbol in Config.SYMBOLS:
            ...
            try:
                ...
                if not htf_aligned:
                    ...
                    entry, sl, tp = self._calculate_trade_params(df_15m, exec_a["direction"])
                ...
                if not pullback_continuation_confirmed:
                    failed_confirmations.append("Pullback Continuation Not Confirmed")
                    entry, sl, tp = self._calculate_trade_params(df_15m, exec_a["direction"])
                    rejected_setups.append({
                ...
                if confidence >= Config.CONFIDENCE_THRESHOLD and direction != "NEUTRAL":
                    ...
                    if self.execute_signal(symbol, direction, confidence, df_15m, df_30m, df_2h, liquidity_levels, order_blocks_2h):
                        ...
                        failed_confirmations.append("Risk/Reward or HTF OB Conflict")
                        entry, sl, tp = self._calculate_trade_params(df_15m, direction)
                        rejected_setups.append({
    ...
    def _calculate_trade_params(self, df15, direction) -> Tuple[float, float, float]:
        entry = df15["close"].iloc[-1]
        atr = TA.calculate_atr(df15).iloc[-1]
        if atr == 0: return entry, 0, 0
    ...
    def execute_signal(self, symbol, direction, confidence, df15, df30, df2h, all_liquidity_levels, order_blocks_2h) -> bool:
        entry, sl, tp = self._calculate_trade_params(df15, direction)
